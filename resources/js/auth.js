document.addEventListener('DOMContentLoaded', () => {
    ...
    const setCookie = (n, v, d) => {
        let exp = d ? `; expires=${new Date(Date.now() + d * 86400 * 1000).toUTCString()}` : "";
        document.cookie = `${n}=${encodeURIComponent(v)||""}${exp}; path=/; SameSite=Lax`;
    };
    const getCookie = n => {
        const ca = document.cookie.split(';'), key = n + "=";
        for (let c of ca) { c = c.trim(); if (c.indexOf(key) === 0) return decodeURIComponent(c.slice(key.length)); }
        return null;
    };
    ...
    const showMsg = (el, msg, err = true) => { el.textContent = msg; el.className = `message-box visible ${err ? 'message-error' : 'message-success'}`; },
          clearMsg = el => { el.textContent = ''; el.classList.remove('visible','message-error','message-success'); },
          setField = (inp, hint, msg, err) => { hint.textContent = msg; inp.classList.toggle('error', err); hint.classList.toggle('error', err); },
          clearField = (inp, hint) => { setField(inp, hint, '', false); if (hint === passwordHint) hint.textContent = 'Min. 8 chars: upper, lower, number, symbol (!@#$%^&*)'; if (hint === confirmHint) hint.textContent = ''; },
          showStep = s => {
            ...
            [...regStepTabs].forEach((tab, i) => {
              ...
              if(i < s - 1) tab.classList.add('completed'); else if(i === s - 1) tab.classList.add('active');
            });
            [...steps].forEach((st, i) => {
              st.classList.remove('active','completed');
              if(i < s - 1) st.classList.add('completed'); else if(i === s - 1) st.classList.add('active');
            });
            [...conns].forEach((line, i) => { line.classList.remove('active','completed'); if(i < s - 1) line.classList.add('completed'); });
          ...
          resetForm = () => {
            ...
            [nameHint, emailHint, phoneHint, passwordHint, confirmHint, loginEmailHint, loginPasswordHint].forEach(h => { h.textContent = ''; h.classList.remove('error'); });
            passwordHint.textContent = 'Min. 8 chars: upper, lower, number, symbol (!@#$%^&*)';
            strengthMeter.className = 'strength-meter'; clearMsg(registerMsg); clearMsg(loginMsg);
            showStep(1);
          },
          ...
          validateField = (inp, hint, type) => {
            let val = inp.value.trim(), valid = val !== '', msg = valid ? (type==='email' && !validateEmail(val) ? 'Please enter a valid email address' : '') : 'This field is required';
            valid = valid && (type==='email' ? validateEmail(val) : true);
            setField(inp, hint, msg, !valid); return valid;
          },
          validateRegField = (inp, hint, type) => {
            ...
            if(type==='confirmPassword' && valid) { confirmHint.textContent = ''; confirmHint.classList.remove('error'); }
            return valid;
          },
    ...
    mainTabs.forEach(tab => tab.addEventListener('click', function(){
      ...
      if(t==='login') { loginTab.classList.add('active'); registerTab.classList.remove('active'); }
      else { loginTab.classList.remove('active'); registerTab.classList.add('active'); showStep(1); }
      clearMsg(loginMsg); clearMsg(registerMsg);
    }));
    [loginEmail, loginPassword].forEach((el, i) => el.addEventListener('blur', () => validateField(el, i ? loginPasswordHint : loginEmailHint, i ? 'password' : 'email')));
    [fullName, regEmail, phone, regPassword, confirmPassword].forEach((el, i) => {
      const type = ['name','email','phone','password','confirmPassword'][i];
      el.addEventListener('blur', () => validateRegField(el, type==='name'?nameHint: type==='email'?emailHint: type==='phone'?phoneHint: type==='password'?passwordHint:confirmHint, type));
    });
    ...
    loginForm.addEventListener('submit', e => {
      ...
      if (validateLogin()) {
        ...
        let users = [];
        try { users = (getCookie('debug_user_accounts') && JSON.parse(getCookie('debug_user_accounts'))) || []; } catch(e){ users = []; }
        if(users.find(u => u.email === em && u.password === pw)) {
            showMsg(loginMsg, 'Login successful! Redirecting...', false);
            setCookie('debug_login_status','loggedIn', null);
            clearField(loginEmail, loginEmailHint); clearField(loginPassword, loginPasswordHint);
            setTimeout(() => window.location.href = 'http://localhost:5500/main.html', 1500);
        } else showMsg(loginMsg, 'Invalid email or password (debug check).');
      } else showMsg(loginMsg, 'Please check the fields marked in red.');
    });
    nextBtn.addEventListener('click', () => { clearMsg(registerMsg); validateStep1() ? showStep(2) : showMsg(registerMsg, 'Please correct the errors in Step 1.') });
    backBtn.addEventListener('click', () => showStep(1));
    submitBtn.addEventListener('click', () => {
      clearMsg(registerMsg);
      if (validateStep2()) {
        let em = regEmail.value.trim(), pw = regPassword.value, users = [];
        try { users = (getCookie('debug_user_accounts') && JSON.parse(getCookie('debug_user_accounts'))) || []; } catch(e){ users = []; }
        if (users.some(u => u.email === em)) showMsg(registerMsg, 'Email already registered for debugging.');
        else { users.push({ email: em, password: pw }); setCookie('debug_user_accounts', JSON.stringify(users), 30); showStep(3); }
      } else showMsg(registerMsg, 'Please correct the errors in Step 2.');
    });
    loginBtn.addEventListener('click', () => { mainTabs.forEach(tab => { if(tab.getAttribute('data-tab')==='login') tab.click(); }); resetForm(); });
